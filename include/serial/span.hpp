// Copyright (c) 2022-2024 TU Wien Space Team
...
template <typename T>
[[nodiscard]] auto Span(std::vector<T> const &vector) -> std::span<T const>;

template <typename T>
[[nodiscard]] auto Span(std::vector<T> *vector) -> std::span<T>;
